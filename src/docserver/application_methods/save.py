...
def save_documentation(documentation, package: schemas.CreatePackage, provided_permissions=None):
    ...
    try:
        ...
        global_write_permission = db_models.Permission.read_unique(db, dict(scope=config.default_write_permission,
                                                                            operation='write'))
        ...
        with TemporaryDirectory() as td:
            ...
            with open(filename, 'wb') as dest:
                ...
                dest.close()
                db_package = db_models.Package.read_unique(db, package)
                if not db_package:
                ...
                document_version = db_models.DocumentationVersion.update_or_create(db, package, filename, db_package, provided_permissions=provided_permissions)
