...
from docserver.models import Language
from docserver.models import Customer
...
class DocsInformation:
    ...
    def __init__(self):
        self.docs_dir = os.getenv('DOCS_DIR', '/data/www/docs')
    ...
    def register_package(self, metadata):
        ...
        package = Package.query.filter_by(name=metadata['package']).first()
        if package is None:
            # Need to create a package
            print('Creating package')
            language = Language.query.filter_by(name=metadata['language']).first()
            if language is None:
            ...
            customer = Customer.query.filter_by(name=metadata['customer']).first()
            if customer is None:
                print('Creating package')
                customer = Customer(name=metadata['customer'])
                db.session.add(customer)
                db.session.commit()
            print(f'Language {customer}')
            package = Package(name=metadata['package'], repository=metadata['repository'], customer_id=customer.id, language_id=language.id)
    ...
    def validate_metadata(self, metadata):
        ...
        metadata['path'] = os.path.join(secure_filename(metadata['package']), secure_filename(metadata['version']))
        return metadata

    def save_documentation(self, zipfile, metadata):
        metadata = self.validate_metadata(metadata)
        zf = ZipFile(zipfile)
        path = self.make_path(metadata)
...
If you are not redirected automatically, here is the <a href='latest/'>latest documentation</a>""")
    ...
    def make_path(self, metadata):
        required_dir = os.path.join(self.docs_dir, metadata['path'])
        if not os.path.exists(required_dir):
            os.makedirs(required_dir)
        return required_dir

    def make_url(self, metadata):
        return f"/packages/{metadata['path'].replace(os.path.sep, '/')}"
