...
with tempfile.TemporaryDirectory() as td:
    ...
    response = requests.post('http://localhost:8001/api/upload', data=values, headers={'permissions': json.dumps({'permissions': ['com/write']})})
    print(response.content.decode())
    upload_url = response.content.decode().split('Location: ')[1][:-1]
    print(upload_url)
    response = requests.put(upload_url, files={'documentation': ('test-upload.zip', open(zip_fname, 'rb').read())},
    ...
    os.chdir(cwd)
    print('-'*20)
    response = requests.get('http://localhost:8001/api/list', headers={'permissions': json.dumps({'permissions': ['com/read']})})
    result1 = response.json()
    print(result1)
    assert len(result1) == 1
    response = requests.get('http://localhost:8001/api/list', headers={'permissions': json.dumps({'permissions': ['com/read', 'com.test/read']})})
